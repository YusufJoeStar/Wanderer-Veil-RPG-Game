...
public class PlayerDeathAnimator : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (animator == null)
        {
            Debug.LogError("PlayerDeathAnimator: No Animator component found on " + gameObject.name);
        }

        if (deathEffect == null && autoStartDeathEffect)
        {
            Debug.LogWarning("PlayerDeathAnimator: No DeathEffect component found on " + gameObject.name +
